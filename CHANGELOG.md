...
### Added — `skills/research-planner/`（新ビルトインスキル・opt-in）

- **計画のみ**を行うリーンなメタエージェント（AGENTS.md 87行）。研究・コード・論文は**作らない**。
- 手順：
  1. **要件抽出**（目的/制約/成果物/データ/成功基準を構造化、曖昧・矛盾・不足を列挙）
  2. **不足ヒアリング**（不明なら番号付き質問して**停止**、計画は出さない）
  3. **複雑度トリアージ**（単純→固定5ステップ直行 / 複雑→**動的に top-level タスク分解、各リーフは固定5ステップに展開**）← 動的×固定の合わせ技で、複雑入力に対応しつつ非決定性を最小化
  4. **レビュー可能な計画**を出力（タスク・依存・前提・リスク）
  5. **貼れるタスクプロンプト群**を出力（各 ≤600字・制約≤3・具体的・**citation ゴールデンルールを各プロンプトに埋め込み**）
- 運用：planner プロジェクトで複雑プロンプトを投入 → 計画＋タスクプロンプトを得る → **人がレビュー → Co-Scientist プロジェクトに順番に貼る**（--resume で成果引継ぎ）。バックエンドのオーケストレーション改修は不要。
- `backend/src/services/skills.service.ts` の `BUILTIN_SKILLS` に登録（起動時に seed、UI から割当て可能）。

### 設計の位置づけ

- これまでの案（メタプロンプト生成 / 自動 validate→fix / 固定5ステップ）を**1つのプランナーに統合**。複雑度判定により単純・複雑の両方を1入口でカバー。リーフを R40 実証済みの固定5ステップに固定することで、動的分解の非決定性リスクを抑える。
- 既存 Co-Scientist は無改修。planner は**別エージェントとして additive**（CLAUDE.md「Co-Scientist を肥大化させない」と整合）。

### 次段（効果確認後）

手動コピペが煩雑なら、planner 出力を AIRA が解析して**自動順次実行＋ステップ間 validate→fix する sequencer** を後続で追加（v3.17.0 候補）。
