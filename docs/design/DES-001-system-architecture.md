...
- 要件: REQ-CHAT-001〜005, REQ-PROJECT-001〜004, REQ-SKILLS-001〜004,  
  ...
  REQ-ERR-001〜010, REQ-NFR-001〜009
...
- **PATCH 更新セマンティクス** (シークレット保持ルール):
  - env / headers のキーが PATCH リクエストに**含まれない** → 既存値を保持 (変更なし)
  - キーが含まれ値が `null` → そのキーを削除
  - キーが含まれ値が `"***"` → **拒否 (400 Bad Request)**: マスク値をそのまま保存することは禁止
  - キーが含まれ値が通常文字列 → 新しい値で上書き
  - UI は未変更のシークレットフィールドをリクエストから除外し、変更時のみ新しい値を送信する
- ログ出力: env / headers 値は一切ログに出力しない。CLI の stdout/stderr にシークレットが含まれる可能性があるため、ストリーミング前にベストエフォートで redact を行う: `config_json.env` および `config_json.headers` の各値**ならびに解決済み `GITHUB_TOKEN`** を redact 対象リストに含め、**リテラル文字列置換** (正規表現メタ文字をエスケープした上で `RegExp` 構築、または `String.replaceAll`) で `***` に置換する。対象リストは値の長さ降順でソートし、部分文字列の重複マッチを防止する。
  - **ストリーミング安全設計**: チャンク境界でシークレットが分割される可能性があるため、キャリーオーバーバッファ (長さ = `maxSecretLength - 1`) を保持する。各チャンク受信時に `buffer + newChunk` を結合して redact し、末尾 `maxSecretLength - 1` バイトをバッファに保持、残りを emit する。プロセス終了時にバッファ残余を flush (最終 redact 後に emit)
  - 子プロセスの出力は redact 処理を通過するまで UI (WebSocket) にもログにも転送しない。完全な防止は保証しない (エンコード変換・部分文字列等) が、平文漏洩リスクを大幅に低減する
...
サーバーサイドで archiver パッケージを使用して生成する。**ZIP 生成時のセキュリティ**: ワークスペースの直接ディレクトリ走査ではなく、`project_files` テーブルの登録済みファイル一覧を基にアーカイブする。各ファイルを追加する前に `FileService.resolve()` を呼び出し、境界検証・シンボリックリンク拒否を適用する。`lstat` でシンボリックリンク/ジャンクションと判明したエントリはスキップする (ZIP に含めない)。
...
| Windows EPERM (ロックまたは権限、判別不可) | 423 Locked (`resource_busy`) |
| OS コマンド実行失敗 | 500 Internal Server Error |

> **Windows EPERM の扱い**: Windows では他プロセス使用中のファイルに対する操作が `EBUSY` ではなく `EPERM` で返される場合がある。`fs.open(path, 'r')` による判別は信頼性が低い (共有モードによっては読み取りは成功するがオープン/削除は失敗する)。v1.0 では **Windows 上の既存ファイルに対する `EPERM` は一律 `423 Locked` として扱い**、ユーザーに「ファイルが使用中または権限不足です。他のアプリケーションを閉じて再試行してください」と表示する。

### DES-PROGRESS-001: 進行状況・右パネル設計

**対応要件**: REQ-PROGRESS-001〜003, REQ-UI-001

**レイアウト**: 画面右側にデフォルト幅 ~320px のパネルを配置。上部に進行状況 (現行 Run)、中部に Run 履歴、下部にファイル一覧。

**リサイズ/折りたたみ**:
- ドラッグハンドルで幅変更可能 (最小 200px〜最大 50vw)
- 折りたたみトグルボタンで完全非表示/復元 (チャットエリアが全幅に展開)
- 折りたたみ状態は `localStorage` に保存し、次回起動時に復元
- 幅変更は `localStorage` に保存 (debounce 300ms)
...
**`data/` ディレクトリ保護** (シークレット含有ファイル共通):
`data/aira.db` には MCP シークレット (`config_json.env` / `config_json.headers`) が平文で格納されるため、`data/settings.json` と同等の保護が必要。
- POSIX: `data/` ディレクトリを `0700` で作成。`data/aira.db` を `0600` で作成 (SQLite の `-journal` / `-wal` も同ディレクトリ内に生成されるため、ディレクトリ自体の保護が必須)
- Windows: `data/` ディレクトリに NTFS ACL を設定 (所有者のみフルコントロール、継承有効)
- 起動時プリフライト: `data/` ディレクトリおよび `data/aira.db` のパーミッションを検証。不適切な場合は警告表示し、可能なら自動修復 (`chmod` / `icacls`)。修復不可なら起動を中止
