...
- **SPReAD プログラム制約は `assets/spread-policy.md` を唯一の正規ソースとする**（各スキルで定数を再定義しない）
...
- **判定基準**: 各要素はユーザーが **明示的に記述** した場合のみ「充足」とする。研究テーマから **推測・類推** して充足とすることは **禁止**
  - SCOPE: データ **規模**（件数・サンプル数）や対象 **地域**・**期間** が数値付きで明記されていなければ ❌
  - TIMELINE: 180日間内の **マイルストーン** が明記されていなければ ❌
  - CONSTRAINTS: **予算額**・**人員数**・**設備制約** が具体的に明記されていなければ ❌
  - DELIVERABLES: **論文**・**ソフトウェア**・**モデル** 等の成果物 **形態** が明記されていなければ ❌（「〜を可能にする」は目標であり成果物ではない）
- **不足が3要素以上の場合**: 作業を開始せず、**必ず `spread1000-context-collector` スキルを呼び出す**
  ...
  - **research-planner 等の後続スキルへの直接遷移は禁止**（context-collector の完了・承認が必須）
- **不足が2要素以下の場合**: 合理的な仮定を明示した上で作業を開始してよい

### 質問の出力方法（重要）

- **質問は通常のチャットテキストとして出力する**（`ask_user` ツールが利用不可でも質問は必須）
- 質問を出力した後は **即座に停止する**（STOP）。**すべてのツール呼び出しが禁止**:
  - `web_search` の実行
  - `bash` コマンドの実行
  - ファイルの作成・編集（`create`, `edit`）
  - **MCP ツール**（`ToolUniverse-execute_tool`, `ToolUniverse-search_tools` 等すべて）
  - その他のすべてのツール呼び出し
  - 例外は `report_intent` と `skill` のみ
- ユーザーの回答は **次のメッセージ** で届く。回答を待たずに先に進んではならない
- この制約は context-collector がメタプロンプトを生成し承認を得るまで継続する
- メタプロンプトは **3層構造** で `output/{project-name}/meta-prompt.md` に保存する:
  - **Layer 1（確認済み入力）**: ユーザーが明示的に承認した6要素 + Baseline/Gap + Data Readiness
  - **Layer 2（推定コンテキスト）**: Layer 1 から LLM が導出した仮説・候補アプローチ・評価指標（ユーザー確認済みだが事実ではない）
  - **Layer 3（プログラム制約）**: `assets/spread-policy.md` から適用されるハード制約
- 後続スキルは Layer 1 を事実、Layer 2 を仮説、Layer 3 をハード制約として扱うこと
