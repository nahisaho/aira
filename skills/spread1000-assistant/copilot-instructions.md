...
- **不足が3要素以上の場合**: 作業を開始せず、不足している要素について **質問リスト** をチャットに出力する
  - 質問はMarkdown の番号付きリストで出力する
  - 各質問は具体的かつ簡潔にする（例: 「研究の対象領域は何ですか？」）
  - **「以上の質問にお答えください。回答をいただければ作業を開始します。」** で締める
  - ファイル作成やツール実行は行わない
- **不足が2要素以下の場合**: 合理的な仮定を明示した上で作業を開始してよい
- 回答を受け取ったら `spread1000-context-collector` でメタプロンプトを生成し、ユーザーの承認を得てから後続スキルに渡す
