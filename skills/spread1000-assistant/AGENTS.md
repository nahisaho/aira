...
DO:
  ...
  2. 各要素はユーザーが **明示的に記述** した場合のみ「充足」とする
     - 研究テーマの記述から **推測・類推して充足とすることは禁止**
     - 例: 「血液検査データからFHを予測」→ SCOPE は推測可能だが「❌ 不明」とする（データ規模の明記なし）
     - 例: 「〜を可能にする」→ DELIVERABLES は ❌（目標であり成果物形態の明記ではない）
  3. 不明な要素が3つ以上 → spread1000-context-collector を起動
  4. 不明な要素が2つ以下 → 通常の WHEN/DO ルーティングへ進む

⚠️ context-collector 完了後、承認済みメタプロンプトを入力として通常ルーティングに戻る
```

**⛔ ハードゲート**: 不明要素が3つ以上の場合、**判定結果をチャットに出力した後、質問1つのみ出力して即座に停止**:
- すべてのツール呼び出しは禁止（`report_intent` と `skill` のみ例外）
- `web_search`, `bash`, `create`, `edit` は禁止
- **MCP ツールも禁止**（`ToolUniverse-execute_tool`, `ToolUniverse-search_tools` 等すべて）
- ファイル作成・編集は禁止
- 他のスキルへのルーティングは禁止
- **research-planner 等の後続スキルへの直接遷移は禁止**（context-collector の完了・承認が必須前提条件）

唯一の許可アクション: 質問を **通常のチャットテキスト** として1つだけ出力し、即座に停止する（`ask_user` ツール不要）。ユーザーの回答は次のメッセージで届く。
...
0. ユーザーのプロンプトに十分なコンテキストがあるか？（6要素中4つ以上が **ユーザーにより明示的に記述** されている）
   - **判定基準**: 推測・類推で充足とすることは禁止。ユーザーが直接言及していない要素は ❌
   - NO（不明3つ以上） → `spread1000-context-collector`（1問1答で収集 → メタプロンプト生成 → 承認後に再分類）
   - YES → next
