...
**Mandatory second-pass loop** (v4.8.0): before delivering the final response —
1. `POST /api/projects/:id/validate`
2. If `pass: true` → done.
3. If `pass: false` → `POST /api/projects/:id/validate/repair` → read the markdown repair_prompt → apply fixes (add missing `[cell:<id>]`, fix bad cell ids, re-execute `[cell:aira-env]` / `[cell:aira-seed]`, etc.) → go back to step 1.
4. Cap: 3 repair iterations. If still failing, **state the remaining failures explicitly in the final response** — do not hide them.
